...
class TestRealtimeExtraction:
    ...
    def test_creates_a_realtime_workflow_and_waits_for_preview(self, realtime_client):
        ...
        try:
            for url in urls:
                ...
                extraction = realtime_client.extract(
                    ExtractOptions(
                        urls=[url],
            ...
            if realtime_client.realtime:
                realtime_client.realtime.on_event(on_event)
            ...
            if workflow_id and realtime_client:
                try:
                    realtime_client.workflow.delete(workflow_id)
                except Exception as error:
                    # Log but don't fail test if cleanup fails
                    print(f"Failed to cleanup workflow {workflow_id}: {error}")
